...
def main(args):
	...
	total_loss = tf.losses.sigmoid_cross_entropy(batch_x, output, scope='reconstruction_loss')
	...
	optimizer = tf.train.AdamOptimizer(args.learning_rate, beta1=0.9)

	update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
	with tf.control_dependencies(update_ops):
		train_tensor = optimizer.minimize(total_loss, var_list=model_vars)
